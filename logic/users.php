...
class users extends boiler
{
    ...
    public function  defaultb($start = 0)
    {
        ...
        $uid = $this->auth->uid;
        $list = $this->db->query("SELECT * FROM users WHERE type IN(5,4) ORDER BY uid DESC LIMIT $start, 250");
        $num = $list->num_rows;
        if (($start + 25) < $num) {
            $is_more = 1;
        }
    ...
    public function search()
    {
        ...
        $query = "%" . $query . "%";
        $list = $this->db->query("SELECT * FROM users WHERE (fullname LIKE '$query' OR phone LIKE '$query' OR email LIKE '$query' OR username LIKE '$query') AND type IN(4,5)  ");
